...
def texmex_bs(request):
    ...
    fajitas = soup.find(id="mntl-card-list-items_2-0-27")
    menu.append(["Sizzling Steak Fajitas", fajitas.get('href')])
    salad = soup.find(id="mntl-card-list-items_2-0-23")
    menu.append(["Spicy Tex-Mex Salad", salad.get('href')])
    casserole = soup.find(id="mntl-card-list-items_2-0-19")
    menu.append(["King Ranch Chicken Casserole III", casserole.get('href')])
    dip = soup.find(id="mntl-card-list-items_2-0-1")
    menu.append(["Seven Layer Tex Mex Dip", dip.get('href')])
    pork = soup.find(id="mntl-card-list-items_2-0-2")
    menu.append(["Calabacita con Puerco (Pork with Squash)", pork.get('href')])
    chicken = soup.find(id="mntl-card-list-items_2-0-36")
    menu.append(["Green Chile Chicken Casserole", chicken.get('href')])
    content = {"menu": menu}
